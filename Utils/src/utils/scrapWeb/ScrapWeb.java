...
//		procesaJugadores();
		...
		private static ProcesadoLaLiga pLaLiga = new ProcesadoLaLiga();
		private static void cargaDatosJugador( String codigoJugador ) {
			ArrayList<String> descripciones = new ArrayList();
			ArrayList<String> cabeceras = new ArrayList();
			HashMap<String,ArrayList<String>> tabla = new HashMap<>();
			String urlLaLiga = "http://www.laliga.es/jugador/" + codigoJugador + "/partidos";
			procesaWeb( urlLaLiga, pLaLiga );
				descripciones.addAll( pLaLiga.getDescs() );
				cabeceras.addAll( pLaLiga.getCabs() );
				for (String jornada : pLaLiga.getTabla().keySet()) {
					ArrayList<String> datos = pLaLiga.getTabla().get( jornada );
					tabla.put( jornada, datos );
				}
			pLaLiga.reset();
			urlLaLiga = "http://www.laliga.es/jugador/" + codigoJugador + "/partidos/defensivas";
			procesaWeb( urlLaLiga, pLaLiga );
				descripciones.addAll( pLaLiga.getDescs() );
				cabeceras.addAll( pLaLiga.getCabs() );
				for (String jornada : pLaLiga.getTabla().keySet()) {
					ArrayList<String> datos = pLaLiga.getTabla().get( jornada );
					if (tabla.containsKey(jornada)) tabla.get(jornada).addAll( datos );
					else tabla.put( jornada, datos );
				}
			pLaLiga.reset();
			urlLaLiga = "http://www.laliga.es/jugador/" + codigoJugador + "/partidos/disciplina";
			procesaWeb( urlLaLiga, pLaLiga );
				descripciones.addAll( pLaLiga.getDescs() );
				cabeceras.addAll( pLaLiga.getCabs() );
				for (String jornada : pLaLiga.getTabla().keySet()) {
			...
			pLaLiga.reset();
			urlLaLiga = "http://www.laliga.es/jugador/" + codigoJugador + "/partidos/ofensivas";
			procesaWeb( urlLaLiga, pLaLiga );
				descripciones.addAll( pLaLiga.getDescs() );
				cabeceras.addAll( pLaLiga.getCabs() );
				for (String jornada : pLaLiga.getTabla().keySet()) {
			...
			pLaLiga.reset();
			urlLaLiga = "http://www.laliga.es/jugador/" + codigoJugador + "/partidos/eficiencia";
			procesaWeb( urlLaLiga, pLaLiga );
				descripciones.addAll( pLaLiga.getDescs() );
				cabeceras.addAll( pLaLiga.getCabs() );
				for (String jornada : pLaLiga.getTabla().keySet()) {
			...
			// Ahora se saca a consola. Cambiar por lo que se desee
			System.out.println( "   " + descripciones.size() + " : " + descripciones );
			System.out.println( "   " + cabeceras.size() + " : " + cabeceras );
			for (int jornada=1; jornada<=38; jornada++) {
				String jornadaString = "" + jornada;
				if (tabla.containsKey(jornadaString)) {
					ArrayList<String> datos = tabla.get( jornadaString );
					System.out.println( jornada + " --> " + datos.size() + " : " + datos );
				}
			}
		...
		private static class ProcesadoLaLiga implements ProcesadoWeb {
			...
			private boolean hayTDreciente = false;
			...
			public void reset() {
				...
				vals.clear();
				tabla.clear();
				haHabidoValores = false;
				sacadasCabeceras = false;
				hayTDreciente = false;
				lastTH = "";
			}
			@Override
			...
			public void procesaTag(Tag tag, LinkedList<Tag> pilaTags) {
				...
				} else if (tag.getTagName().equals("TD")) {
					// Marca que viene un dato a continuación
					hayTDreciente = true;
				}
			}
			@Override
